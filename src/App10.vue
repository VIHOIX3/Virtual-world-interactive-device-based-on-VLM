...
class ModelDissolveEffect {
  ...
  // 加载模型并设置溶解效果
  async loadModel(modelPath, scaleFactor = 1.0) {
    try {
      // 加载模型
      ...
      console.log(this.mesh.layers,this.particleMesh.layers)
      return this.mesh;
    } catch (error) {
  ...
  initParticleSystem(geometry,yshift, scaleFactor) {
    ...
    for (let i = 0; i < this.particleCount; i++) {
      ...
      let y = i * 3 + 1;
      let z = i * 3 + 2;
      // console.log(this.particleCurrPosArr[z])
...
// 创建网格
  ...
  //     scene.add(cube);
  //     const pnum = 10;
  //     const pos = new THREE.Vector3(cube.position.x - 5, 0, cube.position.z);
  //     mount_smokeParticles = createSmoke(pos, pnum, 15, 1.2, 15, 0.3);
  //     for (let i = 0; i < pnum; i++) {
  //       scene.add(mount_smokeParticles[i]);
  //     }
  //   });
  // })
  ...
  // 加载模型
  dissolveEffect = new ModelDissolveEffect(scene, renderer, camera);
  dissolveEffect.loadModel('test13.obj', 10.0)
    .then(() => {
      console.log('模型加载完成');
...

  {
    ...
    // const caourls = import.meta.glob('./assets/cao 22/*.obj', {
    //   as: 'url' // 告诉 Vite 返回资源路径（而不是模块）
    // });
    // //加载对应模块
    // for (const path in caourls) {
    //   caourls[path]().then((url) => {
    //     // console.log('加载模型路径:', url);
    //     model_list.push(loadOneWithMat(url, 'src/assets/cao 22/cao22.jpg', 'grass', 7.5, 3,0,Math.PI));
    //   });
    // }
...
function loadOne(file, ypos) {
  const stone_objLoader = new OBJLoader();
  const textureLoader = new THREE.TextureLoader();
  const customTexture = textureLoader.load('src/assets/zihua/zihua.jpg');
  stone_objLoader.load(file, (object) => {
    if (object.children.length > 0) {
      let model = object;
...
//烟雾生成
function createSmoke(targetPosition, pnum, rangex, rangey, rangez, opa) {
  const loader = new THREE.TextureLoader();
  const smokeTexture = loader.load("Smoke-Element.png");
  const smokeMaterial = new THREE.MeshLambertMaterial({
    color: new THREE.Color("rgb(234,236,237)"),
    map: smokeTexture,
    transparent: true,
    opacity: opa,
  ...
  for (let p = 0; p < pnum; p++) {
    let particle = new THREE.Mesh(geometry, smokeMaterial);
    particle.position.set(
      targetPosition.x + Math.random(-1, 1) * rangex,
      targetPosition.y + Math.random(-1, 1) * rangey,
      Math.random(-1, 1) * rangez + targetPosition.z
    );
